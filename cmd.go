...
func usage() {
	...
	kpi <package(s)>
	----------------------------------------------------------
	| Welcome to kpi the official package manager for KOOMPI |
	----------------------------------------------------------
	V0.0.1
	
Basic usage
	-------------------------------------------------
	| kpi -i <Package name>	=> Install packages	 |	
	| kpi -R <Package name>	=> Uninstall packages    |
	-------------------------------------------------					
	
Common usage
	-------------------------------------------------
	| kpi -S <Package name>	=> Install packages	 |	
	| kpi -R <Package name>	=> Uninstall packages    |
	-------------------------------------------------	

Adavance usage && operations:
    kpi {-h --help}
    kpi {-V --version}
    kpi {-D --database}    <options> <package(s)>
    kpi {-F --files}       [options] [package(s)]
    kpi {-Q --query}       [options] [package(s)]
    kpi {-R --remove}      [options] <package(s)>
    kpi {-S --sync}        [options] [package(s)]
    kpi {-T --deptest}     [options] [package(s)]
    kpi {-U --upgrade}     [options] <file(s)>

New operations:
    kpi {-Y --kpi}         [options] [package(s)]
    kpi {-P --show}        [options]
    kpi {-G --getpkgbuild} [package(s)]
...
show specific options:
    ...
    -d --defaultconfig    Print default kpi configuration
    -g --currentconfig    Print current kpi configuration
...
If no arguments are provided 'kpi -Syu' will be performed
...
func handleCmd() (err error) {
	...
	// kpi THING
	if cmdArgs.existsArg("rm", "uninstall") {
		err = handleRemove()
		return
	}
...
func handleHelp() error {
	if cmdArgs.op == "Y" || cmdArgs.op == "kpi" {
		usage()
		return nil
	}
	return show(passToPacman(cmdArgs))
...
func handleVersion() {
	fmt.Printf("kpi v%s - libalpm v%s\n", version, alpm.Version())
}
